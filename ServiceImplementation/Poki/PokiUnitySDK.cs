...
public class PokiUnitySDK : MonoBehaviour {
	...
	[DllImport("__Internal")]
	private static extern void JS_PokiSDK_measure(string category, string what, string action);
	[DllImport("__Internal")]
	...
	[DllImport("__Internal")]
	private static extern void JS_PokiSDK_rewardedBreak(string size);
	[DllImport("__Internal")]
	...
	[DllImport("__Internal")]
	private static extern void JS_PokiSDK_getUser();
	[DllImport("__Internal")]
	private static extern void JS_PokiSDK_getToken();
	[DllImport("__Internal")]
	private static extern void JS_PokiSDK_login();
	...
	public delegate void getUserResolvedDelegate(PokiUser user);
 	public getUserResolvedDelegate getUserResolvedCallback;
	public delegate void getUserRejectedDelegate();
 	public getUserRejectedDelegate getUserRejectedCallback;
	public delegate void getTokenResolvedDelegate(string token);
 	public getTokenResolvedDelegate getTokenResolvedCallback;
	public delegate void getTokenRejectedDelegate();
 	public getTokenRejectedDelegate getTokenRejectedCallback;
	public delegate void loginResolvedDelegate();
 	public loginResolvedDelegate loginResolvedCallback;
	public delegate void loginRejectedDelegate();
 	public loginRejectedDelegate loginRejectedCallback;
	...
	public void measure (string category, string what = "", string action = "") {
		#if UNITY_EDITOR
		Debug.Log($"PokiUnitySDK: measure({category}, {what}, {action})");
		#else
		JS_PokiSDK_measure(category, what, action);
		#endif
	}
	...
	public void rewardedBreak(string size = ""){
		isShowingAd = true;
		#if UNITY_EDITOR
	...
	public void getUser(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getUser");
		#else
		JS_PokiSDK_getUser();
		#endif
	}
	...
	public void getToken(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getToken");
		#else
		JS_PokiSDK_getToken();
		#endif
	}
	...
	public void getUserResolved(string json){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getUser resolved, received json:"+json);
		#else
		PokiUser user;
		try { user = JsonUtility.FromJson<PokiUser>(json); } catch { user = null; }
		getUserResolvedCallback(user);
		#endif
	}

	public void getUserRejected(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getUser rejected");
		#else
		getUserRejectedCallback();
		#endif
	}

	public void getTokenResolved(string token){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getToken resolved, received token:"+token);
		#else
		getTokenResolvedCallback(token);
		#endif
	}

	public void getTokenRejected(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: getToken rejected");
		#else
		getTokenRejectedCallback();
		#endif
	}

	public void loginResolved(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: login resolved");
		#else
		loginResolvedCallback();
		#endif
	}

	public void loginRejected(){
		#if UNITY_EDITOR
		Debug.Log("PokiUnitySDK: login rejected");
		#else
