mergeInto(LibraryManager.library, {
	...
	JS_PokiSDK_measure: function (category, what, action) {
		PokiSDK.measure(window.properUnityStringify(category), window.properUnityStringify(what), window.properUnityStringify(action));
	},
	...
	JS_PokiSDK_rewardedBreak: function (_size) {
		const size = window.properUnityStringify(_size);
		if(size){
			window.rewardedBreak({size});
		} else {
	...
	JS_PokiSDK_getUser:function(){
		window.getUser();
	},
	JS_PokiSDK_getToken:function(){
		window.getToken();
	},
	JS_PokiSDK_login:function(){
		window.login();
	}
  });
